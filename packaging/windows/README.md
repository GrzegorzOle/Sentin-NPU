...
From 0.4.2 they draw an icon. `sentin-ui.exe` carries one as a resource, which is also what Explorer
and the uninstall list show; before that the desktop shortcut drew the blank sheet Windows gives an
executable with no icon, which on a desktop is hard to tell from a broken link. The icon is
generated by `tools/make_icon.py` rather than committed as a hand-drawn file, so a change to it
reviews as a diff instead of as "binary files differ".
