...
;
; Build:  iscc /DVersion=0.4.4 /DPayload=..\..\dist\sentin-npu-diag-0.4.4-windows-x64 sentin-npu.iss
...
; The console, not the gateway: it is the only binary carrying an icon resource, and it is also the
; thing a person recognises. Pointing this at the gateway drew the blank sheet Windows gives an
; executable with no icon, in the list people scroll to uninstall something.
UninstallDisplayIcon={app}\sentin-ui.exe
; Relative to this script, like LicenseFile. Generated by tools/make_icon.py, which also writes the
; window's own icon from the same drawing so the two cannot part company.
SetupIconFile=sentin-npu.ico
